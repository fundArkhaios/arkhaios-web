...
export default function JoinFund({ symbol, members, requests, fundID, inJournals, completedJournals }) {
  ...
  useEffect(() => {
    const completedByUser = completedJournals.filter(journal => journal.user === user?.accountID);
  ...
  const requestJoin = async (event) => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        setAlert({ message: "Request failed! Try again later...", type: "error" });
      }
    } catch (error) {
...
function PendingRequest({ symbol }) {
  return <p className="pending-request">Your request to join {symbol} is pending.</p>;
}

function Alert({ message, type }) {
  return (
    <div className={`toast toast-center rounded-sm pb-20 top-toast ${type === 'error' ? 'alert-error' : 'alert-success'}`}>
