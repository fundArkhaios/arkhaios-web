...
export const RecoveryContextProvider = ( {children} ) => {
    ...
    }, [sendEmailPage, codeRecoveryPage, newPasswordPage, recoveryCodeError, passwordRecoveryError]);
    ...
    const renderEmailError = () => {
        if (emailError) {
            return (
                <div role="alert" className="alert alert-error">
                    <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0  24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    ...
    const emailComponent = () => {
            return (
            <form id="recovery" onSubmit={sendRecoveryCode} className="flex flex-col justify-center gap-4 px-10 py-10 lg:px-16">
                {renderEmailError()}
    ...
    const codeRecoveryComponent = () => {
        return ( 
            <form id ="checkRecoveryCode" onSubmit={checkRecoveryCode} className = "flex flex-col justify-center gap-4 px-10 py-10 lg:px-16">
                    {renderCodeRecoveryError()}
                    <div className="form-control">
                            <label className="label" htmlFor="recoveryCode"><span className="label-text">Recovery Code</span></label>
                            <input
                                type="text"
                                placeholder=""
                                className="input input-bordered [&:user-invalid]:input-warning [&:user-valid]:input-success"
    ...
    const newPasswordComponent = () => {
        return (
            <form id="resetPassword" onSubmit={resetPassword} className="flex flex-col justify-center gap-4 px-10 py-6 lg:px-16">
